...
// Package chaos implements the proof-of-stake-authority consensus engine.
package chaos
...
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Chaos {
	// Set any missing consensus parameters to their defaults
	conf := *chainConfig.Chaos
	if conf.Epoch <= 1 {
...
func (c *Chaos) GetDb() ethdb.Database {
	return c.db
}

func (c *Chaos) SetChain(chain consensus.ChainHeaderReader) {
	c.chain = chain
}
...
func (c *Chaos) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Chaos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Chaos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Chaos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Chaos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Chaos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Chaos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Chaos) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Chaos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Chaos) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (c *Chaos) tryLazyPunish(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	continuousBlocks := c.chainConfig.ChaosContinuousInturn(header.Number)
...
func (c *Chaos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: val}, accounts.MimetypeChaos, ChaosRLP(header))
	if err != nil {
		return err
	}
	...
	log.Debug("chaos delay...", "duration", common.PrettyDuration(delay))
	go func() {
		select {
		case <-stop:
			return
...
func (c *Chaos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Chaos) SealHash(header *types.Header) common.Hash {
...
func (c *Chaos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "chaos",
		Version:   "1.0",
		Service:   &API{chain: chain, chaos: c},
		Public:    false,
	}}
}
...
// ChaosRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func ChaosRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
