...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, nil, ethash.NewFakeFailer(uint64(len(headers)-1)), vm.Config{}, nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, nil, ethash.NewFakeDelayer(time.Millisecond), vm.Config{}, nil, nil)
	defer chain.Stop()
