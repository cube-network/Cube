...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, nil, params.AllEthashProtocolChanges, nil, engine, vm.Config{}, nil, nil)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func TestLogRebirth(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(db, nil, gspec.Config, nil, engine, vm.Config{}, nil, nil)
	)
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, nil, params.TestChainConfig, nil, engine, vm.Config{}, nil, nil)
	if err != nil {
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec.Config, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(chaindb, nil, params.TestChainConfig, nil, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func TestTransactionIndices(t *testing.T) {
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, nil, ethash.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func TestDeleteRecreateSlots(t *testing.T) {
	...
	diskdb := rawdb.NewMemoryDatabase()
	gspec.MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, nil, params.TestChainConfig, nil, engine, vm.Config{
		Debug:  true,
...
func TestEIP2718Transition(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, nil, gspec.Config, nil, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	if n, err := chain.InsertChain(blocks); err != nil {
