...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, nil, &proConf, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, nil, &conConf, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, nil, &conConf, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, nil, &proConf, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
