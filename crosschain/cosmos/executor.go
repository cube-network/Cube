...
const (
	ExecutorModeExec  ExecutorMode = 0
	ExecutorModeCheck ExecutorMode = 1
	ExecutorModeQuery ExecutorMode = 2
)
...
func makeCosmosHeader(cubeheader *cubetypes.Header, config *params.ChainConfig) *tenderminttypes.Header {
	...
	header := &tenderminttypes.Header{
		...
		ChainID:            MakeChainID(config.ChainID),
...
func NewCosmosExecutor(datadir string,
	...
	impl := NewCosmosStateDB(makeContext(blockContext, config, header, statedb))

	var app *CosmosApp = nil
	db := &DB{}
	db.SetImpl(impl)
...
func (c *Executor) IsCrossChainContract(addr common.Address) bool {
	return IsCrossChainContract(addr)
}
...
func (c *Executor) CheckLoad() {
	if c.mode == ExecutorModeCheck && !c.isLoad {
		c.isLoad = true
		c.Load(c.header.Number.Int64() - 1)
	}

	hdr := makeCosmosHeader(c.header, c.config)
...
func (c *Executor) BeginBlock(header *types.Header, statedb *state.StateDB) {
	...
	log.Info("begin block", "height", header.Number.Int64(), " root ", header.Root.Hex(), " parenthash ", header.ParentHash.Hex())

	c.header = header
	c.statedb = statedb

	ctx := makeContext(c.blockContext, c.config, c.header, c.statedb)
	c.db.SetContext(ctx, c.config)

	c.isLoad = true
	if c.mode == ExecutorModeQuery {
		c.Load(header.Number.Int64())
		hdr := makeCosmosHeader(header, c.config)
		c.app.BeginBlock(abci.RequestBeginBlock{Header: *hdr.ToProto()})
	} else if c.mode == ExecutorModeCheck {
		//lazy load
		c.isLoad = false
	} else {
...
func (c *Executor) EndBlock() {
	// TODO if lazy load version, empty,

	// log.Debug(string(debug.Stack()))
	rc := c.app.BaseApp.Commit()
	if c.header.Number.Int64() > MAX_IAVL_COUNT+c.config.CrosschainCosmosBlock.Int64() {
		key := fmt.Sprintf("s/%d", c.header.Number.Int64()-MAX_IAVL_COUNT)
	...
	log.Info("EndBlock", "ibcHash", hex.EncodeToString(rc.Data[:]))
}
...
func (c *Executor) InitGenesis(evm *vm.EVM, vals []params.CosmosValidator) {
	...
	log.Info("init statedb with code/account")
	...
	c.app.InitChain(abci.RequestInitChain{Time: time.Time{}, ChainId: MakeChainID(c.config.ChainID), InitialHeight: init_block_height})
...
func (c *Executor) Load(init_block_height int64) {
	log.Info("load version...", "height", init_block_height)
	if err := c.app.LoadVersion2(init_block_height); err != nil {
		log.Error("load version failed!!", err.Error())
		panic(err.Error())
	}
}
...
func (c *Executor) Run(evm *vm.EVM, input []byte, suppliedGas uint64) ([]byte, error) {
	...
	for i, msg := range msgs {
		if handler := c.app.MsgServiceRouter().Handler(msg); handler != nil {
			...
			msgResult, err := handler(c.app.GetContextForTx(evm.EstimateMode).WithEvm(evm).WithGas(suppliedGas), msg) /*TODO statedb stateobject wrapper */
			if err != nil {
